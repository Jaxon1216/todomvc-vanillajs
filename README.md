...
> ⚠️ **注意**：由于持续修复 bug 和优化体验，教程文档中的部分代码可能与当前源码略有差异。遇到不一致时，请以源码为准，或使用 AI 工具辅助理解。建议边看教程边对照源码学习，理解核心思路比记忆具体代码更重要！
...
- 在线体验：[todo.jiangxu.net](todo.jiangxu.net)
| 模块 | 功能描述 | 学习重点 |
|------|----------|----------|
...
2. **打开项目**
   - 找到项目根目录的 `index.html`
   - 双击打开（或右键 → 用浏览器打开）
   - 🎉 完成！你应该能看到左侧浅蓝色侧边栏和右侧白色内容区

3. **开始使用**
   - 点击侧边栏切换功能（Todo List / 倒数日 / 路线图）
   - 添加你的待办事项和重要日期
   - 数据自动保存，刷新页面不会丢失
...
1. 用 VS Code 打开项目文件夹
2. 阅读 `docs/basic.md` 开始基础篇教程
3. 参考 `index.html`、`app.js`、`style.css` 中的代码
4. 边学边验证，理解每一行代码的作用

> 💡 **提示**：所有源码都包含详细注释，适合边看边学。遇到问题可以查看对应的教程文档。

---

## 📚 教程目录

本教程分为四个部分，由浅入深，帮助你理解项目的实现原理：
...
📄 **检测链接**：[docs/quiz.md](docs/quiz.md)
...
└── docs/           # 教程文档目录
    ├── basic.md      # 基础篇：Todo List 教程
    ├── countdown.md  # 进阶篇1：倒数日教程
    ├── timeline.md   # 进阶篇2：路线图教程
    ├── responsive.md # 进阶篇3:响应式布局教程
    └── quiz.md       # 学习检测手册
```
